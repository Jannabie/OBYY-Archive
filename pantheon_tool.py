...
# === KONFIGURASI ===
FILENAME = "exec.dat"
OUTPUT_TXT = "dump_v7_newline.txt"
...
def extract_text():
    ...
    while i < total_len:
        if data[i] != 0:
            ...
            # Hitung Padding (Ruang kosong setelah teks)
            padding_count = 0
            peek_i = i
            ...
            if len(temp_bytes) >= MIN_LENGTH and is_valid_shiftjis(temp_bytes):
                try:
                    ...
                    # Convert byte newline to string representation '\n'
                    text_display = text.replace('\n', '\\n').replace('\r', '\\r')
                    
                    extracted.append(f"# [INFO] BATAS: {max_capacity} bytes")
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as f:
        ...
        f.write("# FITUR BARU: Gunakan '\\n' untuk baris baru (Enter).\n")
        f.write("# PERINGATAN: '\\n' memakan 1 byte. Pastikan tidak melebihi BATAS bytes.\n")
...
def validate_text():
    print(f"[*] Memeriksa file terjemahan {OUTPUT_TXT}...")
    ...
    for line in lines:
        ...
        try:
            ...
            print(f"[ERROR] Baris {offset}: Karakter tidak support!")
            error_count += 1
            continue

        if len_new > max_limit:
            print(f"[KEPANJANGAN] Offset {offset}")
            print(f"   Input: {new_text_raw}")
            print(f"   Pakai: {len_new} bytes | Batas: {max_limit} bytes")
            print(f"   -> KURANGI {len_new - max_limit} HURUF LAGI!")
            error_count += 1
    
    if error_count == 0:
        print("\n[HIJAU] SEMUA AMAN! Silakan lanjut ke menu Insert.")
    else:
        print(f"\n[MERAH] Ada {error_count} error. Persingkat kalimatmu!")

def insert_text():
    print("[*] Mulai proses patching...")
    ...
    for line in lines:
        ...
        data[offset + len(new_bytes)] = 0x00 # Null terminator
    ...
    print(f"[SELESAI] Sukses: {patched}, Skip (Kepanjangan): {skipped}")

if __name__ == "__main__":
    print("=== PANTHEON V7 (NEWLINE EDITION) ===")
    print("1. Extract")
    print("2. Validasi")
    print("3. Insert")
    c = input("Pilih: ")
